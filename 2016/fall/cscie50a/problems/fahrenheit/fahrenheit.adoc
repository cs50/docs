...
Depending on where you live in the world, your country uses one of two major temperature scales. If you live in the United States (and don't spend most of your day working in a science lab), it's likely that you're familiar with the Fahrenheit scale, and so if you hear that it's 30 degrees; outside you're probably putting on a heavy coat and warm pants and bracing yourself for the possibility of snow, because that means it's quite cold, given that the freezing point of water is 32 degrees. On the other hand, it's not until the temperature hits 212 degrees that water boils.

In practically every other country of the world (or if you live in the United States and do in fact spend most of your day working in a science lab), you're probably most familiar with the Celsius or centigrade scale. In that case, if you hear that it's 30 degrees outside you're probably going to rummage through your closet for a swimsuit and put on some sunblock, because it's a beautiful beach day. On the Celsius scale, 0 degrees is the freezing point of water, and 100 degrees is the boiling point.
...
Let's do a quick test to make sure things work as expected. Worldwide, the commonly accepted value for normal human body temperature is 37 degrees C. If we plug "37" into that formula where degrees C goes and do the math (37 multiplied by 9 is 333, 333 divided by 5 is 66.6, 66.6 + 32 is 98.6) we get 98.6 degrees F which is what folks in the United States know as normal human body temperature. So that checks out. Similarly if we plug in 0 degrees C (the freezing point of water) into that formula does it convert to 32 degrees F, and 100 degrees C (the boiling point of water) is apparently equivalent to 212 degrees F. Seems like things are going well.
...
If you have it from earlier, take a look at your `hello.c` file, which should be somewhere in your `week1` directory. (If you don't have it, create it now.) The contents should look probably like this:
...
----
~/workspace/week1 $ [underline]#./hello#
...
[source,bash,subs=quotes]
----
~/workspace/week2 $ [underline]#./fahrenheit#
C: [underline]#100#
F: 212.0
----
...
If you pass all the `check50` test cases, and get a green smiley face when you run things through `style50`, congratulations! If not, don't worry: it's just time for a little bit of debugging and/or cleaning up your code. Be sure to print out the message exactly as we do, starting with `F: ` and then printing the converted temperature to exactly one decimal place!
...
To submit your assignment, please do the following by *Sat 9/17 at noon*.

. Open up CS50 IDE.
. Navigate to your `~/workspace/week2` directory, ensuring that `fahrenheit.c` exists therein.
. At the terminal, type `zip first_last.zip fahrenheit.c`, replacing your own first and last name with `first` and `last` respectively. (So, for instance, `john_harvard.zip`)
...
. Head to https://www.dropbox.com/request/u3OzyRssmlNUq6cDtdse[this link] to upload your ZIP file containing `fahrenheit.c`!
