...
Navigate to your `~/workspace/week9` directory. Then execute the following steps in sequence:
...
To submit your assignment, please do the following by *Sat 11/12 at noon*.

. Open up CS50 IDE.
. Navigate to your `~/workspace/week9` directory, ensuring that the directory called `fifteen` containing all of your work on this problem exists therein.
. Right-click on the file `first_last.zip` inside of your `week9` directory in the file browser at left (Which was created during the Sort Race problem) and delete it.
. At the terminal, type `zip -r first_last.zip fifteen`, replacing your own first and last name with `first` and `last` respectively. (So, for instance, `john_harvard.zip`)
...
. Head to https://www.dropbox.com/request/AfqoEo5MuNeUxqQnbGW9[this link] to upload your ZIP file containing your implementation of the Game of Fifteen.

This was the Game of Fifteen.
