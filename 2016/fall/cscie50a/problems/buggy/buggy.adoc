...
You may also find a function called `eprintf` of use. You can find out more about it in the https://reference.cs50.net[CS50 Reference].

== Stop Bugging Me!

These five programs seem to act a bit buggy. Let’s go through each one to see what needs to be done.
...
Require them to do the write up in a file called `buggy.txt` in their `buggy` directory, Erin!
...
. Open up CS50 IDE.
. Navigate to your `~/workspace/week8` directory, ensuring that the `buggy` directory containing all of your bugfixes and your `buggy.txt` file exists therein.
