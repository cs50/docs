...
This route we've implemented for you. Notice how it expects a GET parameter called `geo`, which it passes to `lookup` (which is defined in `helpers.py`) for localized news, thereafter returning a JSON array of objects, each of which has two keys: `link` and `title`. 

You can actually see this file in action. Go ahead and visit URLs like

* https://ide50-username.cs50.io:8080/articles?geo=Cambridge,+Massachusetts
* https://ide50-username.cs50.io:8080/articles?geo=02138

or

* https://ide50-username.cs50.io:8080/articles?geo=New+Haven,+Connecticut
* https://ide50-username.cs50.io:8080/articles?geo=06511

where `username` is your own username. You should see (pretty-printed) JSON arrays of articles!

===== `search`

Not much in this route yet. Just an eventual `TODO`!
...
See `readme.txt` (or `US.txt` itself) for clues as to appropriate types for these fields. Don't include an `id` field.
...
Complete the implementation of `/articles` in such a way that it outputs a JSON array of objects, each of which represents an article for `q`, where `q` is passed into `/articles` as a GET parameter. Odds are you'll want to call `lookup`!

To test `/articles`, even before your text box is operational, simply visit URLs like

* `pass:[https://ide50-username.cs50.io/articles?q=02138]`
* `pass:[https://ide50-username.cs50.io/articles?q=06511]`
* `pass:[https://ide50-username.cs50.io/articles?q=90210]`

and other such variants, where `username` is your own username, to see if you get back the JSON you expect.

==== `search`

Complete the implementation of `/search` in such a way that it outputs a JSON array of objects, each of which represents a row from `places` that somehow matches the value of `geo`. The value of `geo`, passed into `/search` as a GET parameter, meanwhile, might be a city, state, and/or postal code. We leave it to you to decide what constitutes a match and, therefore, which rows to `SELECT`. It suffices to support searching by postal codes only, but try to support searching by city and/or state as well. Odds are you'll find SQL's `LIKE` keyword helpful. If feeling adventurous, you might like (but are not required) to experiment with SQLite's support for https://www.sqlite.org/fts3.html[full-text searches].

For instance, consider the query below.

[source,sql]
----
db.execute("SELECT * FROM places WHERE postal_code = :geo", geo=request.form.get("geo"))
----
...
db.execute("SELECT * FROM places WHERE postal_code LIKE :geo", geo=geo)
----
...
* `pass:[https://ide50-username.cs50.io/search?geo=02138]`
* `pass:[https://ide50-username.cs50.io/search?geo=Cambridge+MA]`
* `pass:[https://ide50-username.cs50.io/search?geo=Cambridge,+MA]`
* `pass:[https://ide50-username.cs50.io/search?geo=Cambridge,+Massachusetts]`
* `pass:[https://ide50-username.cs50.io/search?geo=Cambridge,+Massachusetts,+US]`

or

* `pass:[https://ide50-username.cs50.io/search?geo=06511]`
* `pass:[https://ide50-username.cs50.io/search?geo=New+Haven+CT]`
* `pass:[https://ide50-username.cs50.io/search?geo=New+Haven,+CT]`
* `pass:[https://ide50-username.cs50.io/search?geo=New+Haven,+Connecticut]`
* `pass:[https://ide50-username.cs50.io/search?geo=New+Haven,+Connecticut,+US]`
