...
If seeing this in a `for` loop, be sure you're indeed iterating over a `list` and not, e.g., `None`. In particular, be sure you're checking the return value of `get_user_timeline`, which, per its implementation, can return `None` in cases of error.
