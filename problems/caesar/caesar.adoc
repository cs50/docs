...
* You can assume that, if a user does provide a command-line argument, it will be entirely numeric (e.g., `1`). No need to check that it's indeed numeric.
...
[source,linenums,subs=quotes]
----
if user didn't run program with exactly one command-line argument
    exit
prompt user for plaintext
for each character in plaintext
    if character is alphabetical
        output character rotated by _k_ places
    else
...
Here are some (non-exhaustive) samples of how a user might (try to) use your program. Assumed that the underlined text is what some user has typed.
