...
* Write, in a file called `cash.c` in `~/workspace/unit1/cash/`, a program that first asks the user how much change is owed and then spits out the minimum number of coins with which said change can be made.
...
Ensure that `cash.c` is in `~/workspace/unit1/cash`, as with:

[source]
----
cd ~/workspace/unit1/cash
ls
----

If `cash.c` is not in `~/workspace/unit1/cash`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
