...
* Write, in a file called `cash.c` in `~/chapter1/cash/`, a program that first asks the user how much change is owed and then spits out the minimum number of coins with which said change can be made.
...
* And take care to https://man.cs50.io/3/round[round] your cents (to the nearest penny); don't "truncate" (i.e., floor) your cents!

== How to Submit

=== Step 1 of 2

Ensure that `cash.c` is in `~/chapter1/cash`, as with:
...
If `cash.c` is not in `~/chapter1/cash`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
