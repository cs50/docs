...
Be sure that each of your files are in `~/chapter6/bleep`, as with:
...
If any file is not in `~/chapter6/bleep`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
