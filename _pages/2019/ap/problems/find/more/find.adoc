...
First up, sorting algorithms with Doug:

video::RT-hUXUWQ2I[youtube]

video::3hH8kTHFw2A[youtube]

video::O0VbBkUvriI[youtube]

And then a discussion of linear and binary search with Doug:

video::TwsgCHYmbbA[youtube]

video::T98PIp4omUA[youtube]

== Getting Started

Log into https://ide.cs50.io/[cs50.io] and create a unit3 directory as with
...
Then navigate inside of `unit3` with `cd`. Then execute

[source,bash]
----
wget https://cdn.cs50.net/ap/2019/problems/find/find.zip
----
...
[source,bash]
----
check50 cs50/problems/2019/ap/find/more
----
...
Ensure that `find.c` is in `~/unit3/find`, as with:
...
If `find.c` is not in `~/unit3/find`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
...
----
cd ~/unit3/find
submit50 cs50/problems/2019/ap/find/more
----
+
