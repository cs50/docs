...
[source,subs=quotes]
----
~/workspace/unit1/pennies/ $ [underline]#./pennies#
...
How to begin? Well, as before, create a new file inside of your `~/workspace/unit1/pennies` directory (remember how?) and name it `pennies.c`. Then, fill the file with some "boilerplate" code like the below:
...
Head back to the cs50.io[CS50 IDE] and ensure that `pennies.c` is in `~/workspace/unit1/pennies`, as with:

[source]
----
cd ~/workspace/unit1/pennies
ls
----

If `pennies.c` is not in `~/workspace/unit1/pennies`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
