...
[source,subs=quotes]
----
~/unit1/pennies/ $ [underline]#./pennies#
...
How to begin? Well, as before, create a new file inside of your `~/unit1/pennies` directory (remember how?) and name it `pennies.c`. Then, fill the file with some "boilerplate" code like the below:
...
Of course, if you store the user's amount due in an `int` (which is only 32 bits), the total will be bounded by (2^31^ - 1) pennies. (Why 2^31^ and not 2^32^? And why 1 less than 2^31^?) Best, then, to store your total in a `long`, so that the user benefits from 64 bits. (Don't worry if users' totals overflow 64 bits and even go negative; consider it punishment for greed!)
...
Head back to the ide.cs50.io[CS50 IDE] and ensure that `pennies.c` is in `~/unit1/pennies`, as with:
...
If `pennies.c` is not in `~/unit1/pennies`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
