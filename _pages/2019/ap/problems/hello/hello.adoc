...
Head to https://ide.cs50.io/[ide.cs50.io] and log into CS50 IDE. You may be prompted (again) for your email address. If so, after providing it, click *Private* under *Hosted workspace*, then choose *Harvard's CS50* under *Choose a template*, and click *Create workspace*.
...
followed by Enter to [underline]##c##hange your [underline]##d##irectory to `~/chapter1/` (ergo, `cd`). You should find that your prompt changes to
...
Head to the https://ide.cs50.io[CS50 IDE] and ensure that `hello.c` is in `~/chapter1/hello`, as with:
...
Visit https://submit.cs50.io[submit.cs50.io], and login with your GitHub credentials. Then, head over to the email account you used to sign up for GitHub and accept the invitation link.

=== Step 3 of 3

Then, head back to the https://ide.cs50.io[CS50 IDE] to submit `hello`, as with:
+
[source]
----
cd ~/chapter1/hello
