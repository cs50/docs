...
Head back to the https://cs50.io[CS50 IDE] and ensure that `hello.c` is in `~/workspace/unit1/hello`, as with:

[source]
----
cd ~/workspace/unit1/hello
ls
----

If `hello.c` is not in `~/workspace/unit1/hello`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
