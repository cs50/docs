...
Start off by opening up CS50 IDE and navigate to your `unit2` directory, as with
...
[source,subs=quotes]
----
~/unit2/ $ [underline]#./calc 8.38 - 5.12#
3.260000
----
...
Notice that unlike many other programs you've likely written up to this point, and just like https://docs.cs50.net/2019/ap/problems/friends/friends.html[Old Friends], users are not entering any information **after** the program has started running. Rather, they are providing all of their input to the program at the command line, before the program has begun.
...
** Be sure that `calc.c` is in `~/unit2/calc/`, as with:
+
[source]
----
cd ~/unit2/calc/
