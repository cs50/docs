...
Ensure that `fahrenheit.c` is in `~/workspace/unit1/fahrenheit`, as with:

[source]
----
cd ~/workspace/unit1/fahrenheit
ls
----

If `fahrenheit.c` is not in `~/workspace/unit1/fahrenheit`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
