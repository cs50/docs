...
If you have it from http://docs.cs50.net/2019/ap/problems/hello/hello.html[Hello], take a look at your `hello.c` file, which should be in your `unit1/hello/` directory. (If you don't have it, create it now.) The contents should look probably like this:
...
[source,bash,subs=quotes]
----
~/unit1/fahrenheit/ $ [underline]#./fahrenheit#
C: [underline]#100#
F: 212.0
----
...
[source,bash]
----
check50 cs50/2019/ap/fahrenheit
----
...
Ensure that `fahrenheit.c` is in `~/unit1/fahrenheit`, as with:
...
If `fahrenheit.c` is not in `~/unit1/fahrenheit`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
