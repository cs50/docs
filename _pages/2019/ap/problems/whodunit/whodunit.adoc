...
Next, navigate to your `~/unit4` directory.  Then execute

[source,bash]
----
wget https://cdn.cs50.net/ap/2019/problems/whodunit/whodunit.zip
----
...
you should see that you now have a file called `whodunit.zip` in your `~/unit4` directory.  Unzip it by executing the below.
...
Incidentally, HTML and CSS (languages in which webpages can be written) model colors in this same way. If curious, see https://en.wikipedia.org/wiki/Web_colors for more details.
...
Be sure that each of your files are in `~/unit4/whodunit`, as with:
...
If any file is not in `~/unit4/whodunit`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
...
----
cd ~/unit4/whodunit/
submit50 cs50/2019/ap/whodunit
----
