...
Next, navigate to your `~/chapter4` directory.  Then execute
...
you should see that you now have a file called `whodunit.zip` in your `~/chapter4` directory.  Unzip it by executing the below.
...
Be sure that each of your files are in `~/chapter4/whodunit`, as with:

[source]
----
cd ~/chapter4/whodunit
ls
----

If any file is not in `~/chapter4/whodunit`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
