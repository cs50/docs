...
1. Execute `mkdir chapter4` to make (i.e., create) a directory called `chapter4` in your home directory.
1. Execute `cd chapter4` to change into (i.e., open) that directory.
1. Execute `wget http://cdn.cs50.net/ap/2019/problems/speller/hashtable/speller.zip` to download a (compressed) ZIP file with this problem's distribution.
...
1. Execute `ls`. You should see a directory called `hashtable`.
1. Execute `cd hashtable` to change into that directory.
1. Execute `ls`. You should see this problem's distribution code:
...
So that you can test your implementation of `speller`, we've also provided you with a whole bunch of texts, among them the script from _La La Land_, the text of the Affordable Care Act, three million bytes from Tolstoy, some excerpts from _The Federalist Papers_ and Shakespeare, the entirety of the King James V Bible and the Koran, and more. So that you know what to expect, open and skim each of those files, all of which are in a directory called `texts` within your `chapter4/speller/hashtable` directory.
...
[source]
----
~cs50/chapter4/speller texts/lalaland.txt
----
...
~cs50/chapter4/speller texts/lalaland.txt > staff.txt
----
...
Be sure that each of your files are in `~/chapter4/speller/hashtable`, as with:

[source]
----
cd ~/chapter4/speller/hashtable
ls
----

If any file is not in `~/chapter4/speller/hashtable`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).

=== Step 2 of 2

* To submit `speller`, execute
