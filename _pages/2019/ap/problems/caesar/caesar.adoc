...
And, with regard to wrapping around from `Z` to `A` (or `z` to `a`), don't forget about `%`, C's modulo operator. You might also want to check out https://www.asciitable.com/, which reveals the ASCII codes for more than just alphabetical characters, just in case you find yourself printing some characters accidentally.
...
** Be sure that `caesar.c` is in `~/unit2/caesar/`, as with:
+
[source]
----
cd ~/unit2/caesar/
