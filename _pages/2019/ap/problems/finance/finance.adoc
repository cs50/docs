...
1. Execute `mkdir chapter8` to make (i.e., create) a directory called `chapter8` in your home directory, if you haven't already done so.
1. Execute `cd chapter8` to change into (i.e., open) that directory.
1. Execute `wget http://cdn.cs50.net/2019/fall/tracks/web/finance/finance.zip` to download a (compressed) ZIP file with this problem's distribution.
1. Execute `unzip finance.zip` to uncompress that file.
1. Execute `rm finance.zip` followed by `yes` or `y` to delete that ZIP file.
1. Execute `ls`. You should see a directory called `finance`, which was inside of that ZIP file.
1. Execute `cd finance` to change into that directory.
1. Execute `ls`. You should see this problem's distribution code, including `application.py`, `helpers.py`, `finance.db`, `requirements.txt`, `static/` and `templates/`.
...
If, upon running `check50`, you see this error, odds are you have a bug in your code somewhere! Open up the submit.cs50.io link produced by `check50` to see a detailed traceback to help you debug!
...
Be sure that each of your files are in `~/chapter8/finance`, as with:
...
If any file is not in `~/chapter8/finance`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
