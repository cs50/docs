...
Create a file called `isbn.c` inside `~/chapter1/isbn`, in which you should write a program that prompts the user for an ISBN-10 and then reports (via `printf`) whether the number's legit. So that we can automate some tests of your code, we ask that your program's last line of output be either `YES\n` or `NO\n`, nothing more, nothing less.
...
Ensure that `isbn.c` is in `~/chapter1/isbn`, as with:
...
If `isbn.c` is not in `~/chapter1/isbn`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
