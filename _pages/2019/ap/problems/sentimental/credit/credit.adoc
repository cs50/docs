...
* In `credit.py` in `~/chapter6/credit/`, write a program that prompts the user for a credit card number and then reports (via `print`) whether it is a valid American Express, MasterCard, or Visa card number, exactly as you did in link:https://lab.cs50.io/cs50/labs/2019/ap/credit/[Problem Set 1], except that your program this time should be written (a) in Python and (b) in CS50 IDE.
...
Head back to the https://ide.cs50.io[CS50 IDE] and ensure that `credit.py` is in `~/chapter6/credit`, as with:
...
If `credit.py` is not in `~/chapter6/credit`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
