...
Focusing on those two languages, create a simple homepage that introduces yourself, your favorite hobby or extracurricular, or anything else of interest to you.
...
1. Execute `mkdir unit5` to make (i.e., create) a directory called `unit5` in your home directory.
1. Execute `cd unit5` to change into (i.e., open) that directory.
1. Execute `wget http://cdn.cs50.net/ap/2019/problems/homepage/hompage.zip` to download a (compressed) ZIP file with this problem's distribution.
...
```
+
in the terminal window and clicking on the link that appears, which should be of the form `http://localhost:XXXX` (with `XXXX` usually being either `8080` or `8081`), which will redirect you to a slightly more complex-looking link of the form `http://SOMETHING.amazonaws.com:XXXX/`. From there, you can click on `index.html` to see your homepage.
...
* Have at least one stylesheet file of your own creation, `styles.css`, which uses at least five (5) different CSS selectors (each of which is either a tag (`example`), class (`.example`), or ID (`#example`) selector), and within which you use a total of at least five (5) different CSS properties, such as `font-size`, or `margin`; and
* Ensure that your site looks nice on browsers both on mobile devices as well as laptops and desktops; but
...
```
http://SOMETHING.amazonaws.com:XXXX/
```

Where `SOMETHING` is a string of words and numbers that uniquely refer to your IDE instance, and `XXXX` is either `8080` or `8081`, those referring to the _port number_ on which your web server is listening for requests (instead of the default `80` or `443` for HTTP and HTTPS requests, respectively). At that address, you'll see a listing of all of the files in your `homepage` directory, and from there can navigate to any HTML page you've created.
...
Considering the following in the overall design of your site:
...
Indeed, because `style50` does not support HTML/CSS, your style score may end up prepopulated in CS50.me upon submitting as much lower than it will ultimately be. Humans ultimately have full discretion over style scores anyway, and the course staff knows they'll likely need to override the value there, so don't be alarmed by it!
...
```
submit50 cs50/2019/ap/homepage
```
