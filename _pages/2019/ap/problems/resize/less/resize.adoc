...
Head back to the ide.cs50.io[CS50 IDE] and ensure that `resize.c` is in `~/chapter3/resize`, as with:
...
If `resize.c` is not in `~/chapter3/resize`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
...
----
cd ~/chapter3/resize
submit50 cs50/problems/2019/ap/resize
----
+
