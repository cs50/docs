...
* Require that a user input a number of shares, implemented as a number field whose `name` is `shares`. Render an apology if the input is not a positive integer.
