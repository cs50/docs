...
1. Execute `cd` to ensure that you're in `~` (i.e., your home directory, which is usually referred to as `~`).
1. Execute `mkdir pset3` to make (i.e., create) a directory called `pset3` in your home directory, if you haven't already.
...
First, be sure you're familiar with the structure of 24-bit uncompressed BMPs, as introduced in link:https://lab.cs50.io/cs50/labs/2019/spring/whodunit[Whodunit].
...
Now look at the underlying bytes that compose `smiley.bmp`. In your terminal window copy and paste the following:

[source,bash]
----
xxd -s 54 -g 3 -c 24 smiley.bmp
----

(No need to worry for now what all the flags and numbers represent!) You should see the below, byte 54 onward of `smiley.bmp`. (Recall that a 24-bit BMP's first 14 + 40 = 54 bytes are filled with metadata, so we're simply ignoring that for now.) As before, we've highlighted in red all instances of `0000ff`.
...
To "see" this padding, in your terminal window copy and paste the following:

[source,bash]
----
xxd -s 54 -g 3 -c 12 small.bmp
----

You should see output like the below; we've highlighted in green all instances of `00ff00`.
...
For contrast, let's next look at `large.bmp`, which looks identical to `small.bmp` but, at 12 pixels by 12 pixels, is four times as large. Execute:

[source,bash]
----
xxd -s 54 -g 3 -c 36 large.bmp
----
...
* Implement your program in a file called `resize.c` inside of `~/pset3/resize/more/`.
...
[source]
----
~cs50/2019/x/pset3/more/peek large.bmp
----
...
~cs50/2019/x/pset3/more/resize 4.0 small.bmp staff.bmp
~cs50/2019/x/pset3/more/peek student.bmp staff.bmp
----
...
[source]
----
check50 cs50/2019/spring/resize/more
----
...
```
submit50 cs50/2019/spring/resize/more
```
