...
----
+
Visit the URL outputted by `flask`. If a directory called `flask_session/` appears in `finance/`, not to worry; that's where Flask stores your session(s).
...
Implement any two (2) personal touches of your choice:

* Allow users to change their passwords.
* Allow users to add additional cash to their account.
* Allow users to buy more shares or sell shares of stocks they already own via `index` itself, without having to type stocks' symbols manually.
* Alter the aesthetics of the sites, as via
