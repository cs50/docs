...
Create a file called `isbn.c` inside `~/workspace/chapter1/isbn`, in which you should write a program that prompts the user for an ISBN-10 and then reports (via `printf`) whether the number's legit. So that we can automate some tests of your code, we ask that your program's last line of output be either `YES\n` or `NO\n`, nothing more, nothing less.
...
[source,subs=quotes]
----
~/workspace/chapter1/isbn/ $ [underline]#./isbn#
ISBN: [underline]#0789751984#
YES
----

Of course, `get_long_long(“ISBN: “)` itself will reject an ISBN-10's hyphens (and more) anyway:
...
Ensure that `isbn.c` is in `~/workspace/chapter1/isbn`, as with:

[source]
----
cd ~/workspace/chapter1/isbn
ls
----

If `isbn.c` is not in `~/workspace/chapter1/isbn`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
