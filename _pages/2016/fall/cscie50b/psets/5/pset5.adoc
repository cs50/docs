...
* Refresh your memory from CSCI E-50a.
...
in order to download a ZIP (i.e., compressed version) of this problem set's distro.footnote:[We are deliberately using the Fall 2015 version of the distribution code from this assignment, because the Fall 2016 version will be different in ways that we did not teach in CSCI E-50a in Spring 2016!] If you then execute
...
To submit your assignment, please do the following by *Fri 9/23 at noon*.

. Open up CS50 IDE.
. Navigate to your `~/workspace` directory, ensuring that `pset5` (and all of its contents) exists therein.
. At the terminal, type `zip -r first_last.zip pset5`, replacing your own first and last name with `first` and `last` respectively. (So, for instance, `john_harvard.zip`)
...
. Head to https://www.dropbox.com/request/JfmbdXoxVRGCyoneIMK7[this link] to upload your ZIP file containing your `pset5` submission!
