...
within a terminal window to make sure your workspace is up-to-date. Next, navigate to your `workspace` directory and create a new directory therein called `week6` (remember how?) and then navigate inside, as with
...
Keep in mind that `~` denotes your home directory, `~/workspace` denotes a directory called `workspace` therein, and `~/workspace/week6` denotes a directory called `week6` within `~/workspace`. Your prompt should now resemble the below.
...
Next create a new file entitled `calc.c` in your `week6` directory.

Though we'll remind you of the existence of this video many times throughout the problems in this and a few future assignments, be sure to have a look at Christopher's short video on command-line arguments.
...
[source,subs=quotes]
----
~/workspace/week6/ $ [underline]#./calc 4 + 5#
9.000000
----
...
[source,subs=quotes]
----
~/workspace/week6/ $ [underline]#./calc 8.38 - 5.12#
3.260000
----
...
Notice that unlike many other programs you've likely written up to this point, users are not entering any information **after** the program has started running. Rather, they are providing all of their input to the program at the command line, before the program has begun.
...
* You needn't worry about floating-point imprecision with your calculator, and you needn't (and indeed shouldn't!) print out the correct answer to more than six decimal places.
...
To submit your assignment, please do the following by *Wed 10/19 at noon*.

. Open up CS50 IDE.
. Navigate to your `~/workspace/week6` directory, ensuring that `calc.c` exists therein.
. At the terminal, type `zip first_last.zip calc.c`, replacing your own first and last name with `first` and `last` respectively. (So, for instance, `john_harvard.zip`)
. This should create a new file called `first_last.zip` in your `week6` directory.
...
. Head to https://www.dropbox.com/request/2hUin1sxDvQYZpiG9pKc[this link] to upload your ZIP file containing `calc.c`!
