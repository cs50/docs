...
within a terminal window to make sure your workspace is up-to-date. Now execute
...
[source,subs=quotes]
----
~/workspace/week7 $ [underline]#./caesar 13#
...
To submit your assignment, please do the following by *Wed 10/26 at noon*.

. Open up CS50 IDE.
. Navigate to your `~/workspace/week7` directory, ensuring that `caesar.c` exists therein.
. Type `rm *.zip` to delete the ZIP file you made for the Initials programs in the `week7` directory.
. At the terminal, type `zip first_last.zip caesar.c`, replacing your own first and last name with `first` and `last` respectively. (So, for instance, `john_harvard.zip`)
...
. Head to https://www.dropbox.com/request/CpMstiE9OYidXqMWKRV8[this link] to upload your ZIP file containing `caesar.c`!
