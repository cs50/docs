...
Don't be alarmed by the length of this problem's specification. Most of it is just instructions for getting your programming environment set up and learning how to navigate within it. Once these steps are completed, you'll be good to go for the rest of the year in CSCI E-50a (and E-50b)!
...
Once you've got your CS50 IDE instance open in a browser window, have a look over the README file we've included with it. It includes some basics about using your new workspace.
...
after which you are free to continue exploring CS50 IDE. As of the time we're writing this specification, the current version of CS50 IDE is *69*, but that's likely to change once or a few times per week. That's the number you should see near the top-right corner of your screen. You can always check the version number of your IDE, also, by typing `version50` in the Terminal.

=== Navigating CS50 IDE

Okay, let's create a folder (otherwise known as a *directory*) in which your code for this problem will soon live. On the left side of the screen you should see a file tree listing the current contents of your CS50 IDE workspace (right now it should simply be a folder entitled `~/workspace`). Right-click in the blank space underneath that folder and select *New Folder* near the bottom of the context menu that pops up. This should create, as expected, a new folder for you entitled *New Folder* which you can rename now to `week1`. If you accidentally leave it named `New Folder`, simply right click on the folder, choose *Rename* from the context menu, and rename it.
...
Let's create a new file to play around with. Right-click on your newly-created `week1` folder and choose *New File* from the context menu. Then, double-click on this new file (which should hopefully be called `Untitled` and which should be nested beneath `week1` in the file tree), and a blank window should open up in Ace with the tab for `Untitled` as the active one.
...
(where `~/workspace` &ndash; i.e., inside a folder called `workspace` which is itself inside your home directory, the shorthand for which is `~` &ndash; is the directory you are currently in.) If that's the case, based on the file tree should be a `week1` directory somewhere inside, since we created it just a few moments ago. Let's confirm as much.
...
and then Enter. That's a lowercase `L` and a lowercase `S`, which is shorthand notation for "list." Indeed, you should then see a list of the folders inside of your workspace directory, among which is `week1`! Let's open that folder! Type
...
followed by Enter to [underline]##c##hange your [underline]##d##irectory to `~/workspace/week1` (ergo, `cd`). You should find that your prompt changes to
...
confirming that you are indeed now inside of `~/workspace/week1` (i.e., a directory called `week1` inside of a directory called `workspace` inside of your home directory). Now type
...
and then Enter. It turns out that if you don't provide `cd` with a "command-line argument" (i.e., a directory's name), it whisks you back to your home directory by default. But remember that everything lives inside of `~/workspace`... so we need to get back there! Just:
...
Shall we have you write your first program? You could go ahead create a new file inside of your `week1` directory and save the file as `hello.c` (not `hello.txt`), just as before (remember how?), but we'll also take this opportunity to show you another way. In the terminal window, type
...
after a few moments, you should see `hello.c` has been created inside of your `week1` directory. Neat! Double-click on that file in the file tree to edit it, as before, then go ahead and write your first program by typing these lines into the file (though you're welcome to change the words between quotes to whatever you'd like):
...
at your prompt, followed by Enter. (Just be sure that you are inside of `~/workspace/week1` within your terminal window, as your prompt will confirm or deny.) If you see no more errors, try running your program by typing
...
If not already there, navigate your way to `~/workspace/week1` (Remember how?)
...
Okay, assuming your file's name is definitely spelled `hello.c` now, go ahead and execute the below. Note that `2016.hello` is just a unique identifier for this problem's checks. We'll always specify exactly the unique identifier you should use to check your programs' correctness.
...
To submit your assignment, please do the following by *Wed 9/14 at 11:59pm*.

. Open up CS50 IDE.
. Navigate to your `~/workspace/week1` directory, ensuring that `hello.c` exists therein.
. At the terminal, type `zip first_last.zip hello.c`, replacing your own first and last name with `first` and `last` respectively. (So, for instance, `john_harvard.zip`)
. This should create a new file called `first_last.zip` in your `week1` directory.
...
. Head to https://www.dropbox.com/request/TdsyOmjZvMoMQFIlfMEn[this link] to upload your ZIP file containing `hello.c`!
