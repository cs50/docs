...
* Sections 1 through 13 of http://tldp.org/HOWTO/NCURSES-Programming-HOWTO/.

== Academic Honesty

This course's philosophy on academic honesty is best stated as "be reasonable." The course recognizes that interactions with classmates and others can facilitate mastery of the course's material. However, there remains a line between enlisting the help of another and submitting the work of another. The essence of all work that you submit to this course must be your own. Collaboration on problems is not permitted (unless explicitly stated otherwise) except to the extent that you may ask classmates and others for help so long as that help does not reduce to another doing your work for you. Generally speaking, when asking for help, you may show your code or writing to others, but you may not view theirs, so long as you and they respect this policy's other constraints.
== Assessment
...
This problem is longer than any you've previously tackled... but that's why you've got a week to work on it. We hope the results will be worth the effort!
...
Then, create a new directory inside of your `workspace` called `week10` (Remember how?) and navigate inside. Once there, obtain a copy of this problem's distro by typing:

[source,bash]
----
wget http://docs.cs50.net/2016/fall/cscie50a/problems/sudoku/sudoku.zip
----

Unzip the ZIP file (remember how?) and then delete the ZIP file from your `week10` directory. Navigate into your newly-created `sudoku` directory and type:
...
Navigate to your `~/workspace/week10/sudoku` directory and execute the increasingly familiar command below:
...
Before you do, though, a few questions for you. Create a file called `questions.txt` in `~/workspace/week10/sudoku` and record in it your answers to the below:
...
Now it's time to augment the game by adding your choice of three (3) of the following five (5) additional features:

* Do not allow the user to alter numbers that "came with" the board.
...
To play with the staff's own implementation of Sudoku, which incorporates all of the features of this assignment, you may execute the below.
...
To submit your assignment, please do the following by *Sat 11/19 at noon*.

. Open up CS50 IDE.
. Navigate to your `~/workspace/week10` directory, ensuring that the directory called `sudoku` containing all of your work on this problem exists therein.
. At the terminal, type `zip -r first_last.zip sudoku`, replacing your own first and last name with `first` and `last` respectively. (So, for instance, `john_harvard.zip`)
. This should create a new file called `first_last.zip` in your `week10` directory.
...
. Head to https://www.dropbox.com/request/YnNVr0X5e8UpPU9N1tT1[this link] to upload your ZIP file containing your completed implementation of Sudoku.
