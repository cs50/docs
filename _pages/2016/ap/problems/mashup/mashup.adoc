...
Your mission is to implement "mashup" that integrates Google Maps with Google News with a MySQL database containing thousands of postal codes, GPS coordinates, and more. Quite like this here version by the staff!
...
[source,bash]
----
wget http://docs.cs50.net/2016/x/ap/problems/mashup/mashup.zip
----

Unzip it into `~/workspace`, so that you ultimately have a `mashup` directory in `~/workspace`. Then delete `mashup.zip`. And then download http://docs.cs50.net/2016/x/ap/problems/mashup/mashup.sql into `~/workspace` as well.

(Remember how?)

Next, execute `ls` within `~/workspace/mashup`, and you should see three subdirectories: `bin`, `includes`, and `public`. Ensure that permissions are as follows:
...
Then (re)start Apache with the below so that it uses `~/workspace/mashup/public` as its root.

[source,bash]
----
apache50 start ~/workspace/mashup/public
----
...
video::ASA8fAEerNo[youtube]

And now a closer look at the distribution code.

=== import

Navigate your way to `~/workspace/mashup/bin` and open up `import`. Not much there yet! Just a shebang and `TODO`. It's in this file that you'll ultimately write a PHP script that iterates over the lines in `US.txt`, ``INSERT``ing data from each into `places`, that MySQL table. But more on that later.
...
Now navigate your way to `~/workspace/mashup/public/css` and open up `styles.css`. In there is a bunch of CSS that implements the mashup's default UI. Feel free to tinker (i.e., make changes, save the file, and reload the page in Chrome) to see how everything works, but best to undo any such changes for now before forging ahead.

=== scripts.js

Navigate next to `~/workspace/mashup/public/js` and open up `scripts.js`. Ah, the most interesting file yet! It's this file that implements the mashup's "front-end" UI, relying on Google Maps and some "back-end" PHP scripts for data (that we'll soon explore). Let's walk through this one.
...
where `username` is your own username. You should see a (pretty-printed) JSON array of articles!

=== config.php

Let's now take a quick peek at the file that all those other PHP files have required. Navigate your way to `~/workspace/mashup/includes` and open up `config.php`. Ah, a file quite like C$50 Finance’s own `config.php`, albeit simpler. It even loads CS50's PHP library, along with `helpers.php`.

=== helpers.php

In this file we've defined just one function, `lookup`. Unlike C$50 Finance’s `lookup`, though, this version of `lookup` queries Google News for articles for a particular geography. No need to understand every one this file's lines, but do review its comments!

=== config.json

Next open up `config.json`. Ah, another familiar sight, albeit with for database called `mashup`.
...
First, tackle those ``TODO``s inside of `config.json`, just as you did for C$50 Finance.
...
Download your sql and Mashup directory and then await further instructions from your teacher on how they want you to submit.
