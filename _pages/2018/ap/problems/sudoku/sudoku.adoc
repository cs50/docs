...
First, log into https://cs50.io/[cs50.io]. Then, create a new directory inside of your `workspace` called `unit4` (Remember how?) and navigate inside. Once there, obtain a copy of this problem's distro by typing:

[source,bash]
----
wget https://cdn.cs50.net/ap/2018/problems/sudoku/sudoku.zip
----
...
Navigate to your `~/workspace/unit4/sudoku` directory and execute the increasingly familiar command below:
...
** Be sure that the following files exist in `~/workspace/unit4/sudoku/`:
* `Makefile`  
* `sudoku.c`  
* `sudoku.h` 
as with:
