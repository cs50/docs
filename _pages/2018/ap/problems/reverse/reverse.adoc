...
then create a new `unit2` directory as with
...
Ensure that `mystery.c` is in `~/workspace/unit2/reverse`, as with:

[source]
----
cd ~/workspace/unit2/reverse
ls
----

If `mystery.c` is not in `~/workspace/unit2/reverse`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
