...
* In `credit.c` in `~workspace/unit1/credit`, write a program that prompts the user for a credit card number and then reports (via `printf`) whether it is a valid American Express, MasterCard, or Visa card number, per the definitions of each's format herein.
...
Retry: [underline]#foo#
Retry: [underline]#4003600000000014#
VISA
----
...
Ensure that `credit.c` is in `~/workspace/unit1/credit`, as with:

[source]
----
cd ~/workspace/unit1/credit
ls
----

If `credit.c` is not in `~/workspace/unit1/credit`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
