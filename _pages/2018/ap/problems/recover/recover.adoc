...
Navigate to your `~/workspace/unit4` directory.  For the first time in a while, no big code distro this time, but there is one important file to download. Execute

[source,bash]
----
wget https://cdn.cs50.net/ap/2018/problems/recover/recover.zip
----
...
Be sure that `recover.c` is in `~/workspace/unit4/recover`, as with:

[source]
----
cd ~/workspace/unit4/recover
ls
----

If any file is not in `~/workspace/unit4/recover`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
