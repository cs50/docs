...
Next, navigate to your `~/workspace/unit4` directory.  Then execute

[source,bash]
----
wget https://cdn.cs50.net/ap/2018/problems/whodunit/whodunit.zip
----
...
you should see that you now have a file called `whodunit.zip` in your `~/workspace/unit4` directory.  Unzip it by executing the below.
...
Be sure that each of your files are in `~/workspace/unit4/whodunit`, as with:

[source]
----
cd ~/workspace/unit4/whodunit
ls
----

If any file is not in `~/workspace/unit4/whodunit`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
...
----
cd ~/workspace/unit4/whodunit/
submit50 cs50/2018/ap/whodunit
----
