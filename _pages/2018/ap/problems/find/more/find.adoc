...
[source,bash]
----
check50 cs50/2018/ap/find/more
----
...
Ensure that `find.c` is in `~/workspace/unit3/find`, as with:
...
If `find.c` is not in `~/workspace/unit3/find`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
...
----
cd ~/workspace/unit3/find
submit50 cs50/2018/ap/find/more
----
+
