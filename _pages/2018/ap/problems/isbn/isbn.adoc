...
Ensure that `isbn.c` is in `~/workspace/unit1/isbn`, as with:
...
If `isbn.c` is not in `~/workspace/unit1/isbn`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
