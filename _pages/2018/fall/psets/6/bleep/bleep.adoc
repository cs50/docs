...
* Prints back the message that the user provided, except if the message contained any banned words, each of its characters is replaced by a `*`.
  * Note: You should not censor words that merely contain a banned word as a substring. For example, **bar** is a banned word in the provided `banned.txt`, but neither __**bar**n__ nor __crow**bar**__ nor __wheel**bar**row__ should be censored.
