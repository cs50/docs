...
Thereafter is `lookup`, a function that, given a `symbol` (e.g., MSFT), returns a stock quote for that symbol in the form of a `dict` with three keys: `name`, whose value is a `str` (e.g., "Netflix, Inc."); `price`, whose value is a `float`; and `symbol`, whose value is a `str`, a canonicalized (uppercase) version of a stock's symbol, irrespective of how that symbol was capitalized when passed into `lookup`. For backwards compatibility with prior versions of this problem set, you'll notice that `lookup` actually tries to query Yahoo! Finance initially, then uses AlphaVantage as a second attempt. Yahoo! officially deprecated their API in November 2017, so all of our queries will ultimately run through AlphaVantage. Also note ultimately that AlphaVantage returns the same data for both `symbol` and `name`; this is indeed the expected behavior and you need not alter it.
