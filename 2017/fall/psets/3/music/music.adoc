...
** or `XYZ` (e.g., `A#4`), where `X` is any of `A` through `G`, `Y` is `#` or `b`, and `Z` is any of `0` through `8`; and
...
In implementing this function, you might find https://reference.cs50.net/math/pow[`pow`], declared in `math.h`, of interest.
...
[source]
check50 cs50/2017/fall/music # starting Sat 9/16 eve
...
As always, when writing code, take baby steps, only implementing enough lines to make progress before testing (and, if need be, debugging) your code. Only once that first step is succesful (i.e., debugged!) should you take another.

In the context of `frequency`, taking baby steps might mean:
