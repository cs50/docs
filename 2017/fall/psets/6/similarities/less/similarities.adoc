...
Open up `application.py`. This file implements a web application that, ultimately, will allow you to run any of those three algorithms on any two text files. No need to understand the entirety of this file, particularly `highlight` and `errorhandler`. But know that the former, given a string, `s`, and a list of other strings, `strings`, highlights (by wrapping them in HTML `span` tags) all instancs of the latter in the former. And the latter ensures that any HTTP errors are displayed on a page of their own.
...
Implement `substrings` in such a way that, given two strings, `a` and `b`, and an `int`, `n`, it returns a `list` of all substrings of length `n` that are, identically, present in both `a` and `b`. The `list` should not contain any duplicates.
