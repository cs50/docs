...
Ensure that `mystery.c` is in `~/workspace/chapter2/reverse`, as with:

[source]
----
cd ~/workspace/chapter2/reverse
ls
----

If `mystery.c` is not in `~/workspace/chapter2/reverse`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
