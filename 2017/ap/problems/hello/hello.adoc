...
Head back to the cs50.io[CS50 IDE] and ensure that `hello.c` is in `~/workspace/chapter1/hello`, as with:
...
If `hello.c` is not in `~/workspace/chapter1/`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
