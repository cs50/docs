...
* In `~workspace/chapter1/credit`, create a file called `credit.c`. In `credit.c`, implement a program that prompts the user for a credit card number and then reports (via `printf`) whether it is a valid American Express, MasterCard, or Visa card number, per the definitions of each's format herein.
...
Ensure that `credit.c` is in `~/workspace/chapter1/credit`, as with:

[source]
----
cd ~/workspace/chapter1/credit
ls
----

If `credit.c` is not in `~/workspace/chapter1/credit`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
