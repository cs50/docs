...
Create a new `calc` directory inside `chapter2` (remember how?). Next create a new file entitled `calc.c` in your `calc` directory.
...
Notice that unlike many other programs you've likely written up to this point, and just like http://docs.cs50.net/2017/ap/problems/friends/friends.html[Old Friends], users are not entering any information **after** the program has started running. Rather, they are providing all of their input to the program at the command line, before the program has begun.
...
** Be sure that `calc.c` is in `~/workspace/pset2/calc/`, as with:
+
[source]
----
cd ~/workspace/chapter2/calc/
ls
----
