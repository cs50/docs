...
[source,bash]
----
wget http://docs.cs50.net/2017/ap/problems/fifteen/fifteen.zip
----
...
Be sure that each of your files are in `~/workspace/chapter3/fifteen`, as with:
...
If any file is not in `~/workspace/chapter3/fifteen`, move it into that directory, as via `mv` (or via CS50 IDE's lefthand file browser).
